...
def render_assessment():
    ...
    if need_reload:    
        ...
        logger.info(f"[DEMO] FORCING NPC regardless of selection - was going to use: {regulation_map.get(st.session_state.selected_country, st.session_state.selected_regulation)}")
        try:
            ...
            if next_clicked:
                ...
                if not unanswered:
                    if is_last_section:
                        ...
                        # AGGRESSIVE CACHE CLEARING - Clear all questionnaire caches
                        st.session_state.clear_questionnaire_cache = True
                        st.session_state.report_cache_cleared = False  # Force report to clear cache too
                        
                        # Clear any LRU cache in assessment module
                        try:
                            import assessment
                            assessment.get_questionnaire.cache_clear()
                            logger.info("[VIEWS] Cleared LRU cache during assessment completion")
                        except Exception as e:
                            logger.warning(f"[VIEWS] Could not clear LRU cache: {e}")
                        
                        # Use correct regulation and industry mapping for final calculation
                        # DEMO MODE: Force NPC for all selections
                        regulation_for_calc = "NPC"
                        logger.info(f"[DEMO] FORCING NPC for calculation regardless of selection")
                        if st.session_state.selected_country == "Qatar" and st.session_state.selected_industry == "General":
                            regulation_for_calc = "NPC"
                        else:
                            regulation_map = {
                        ...
                        logger.info(f"[VIEWS] ASSESSMENT COMPLETION: Final calculation with {regulation_for_calc}/{industry_for_calc}")
                        st.session_state.results = calculate_compliance_score(
                            regulation_for_calc,
                            industry_for_calc
                        )
...
def render_report():
    ...
    if results:
        ...
        if regulation_for_loader == "NPC" and any("PDPPL" in name or "Principles of Data Privacy" in name for name in result_section_names):
            ...
            # DEMO MODE: Force NPC for all selections
            regulation_for_calc = "NPC"
            logger.info(f"[DEMO] FORCING NPC for stale results recalculation")
            if st.session_state.selected_country == "Qatar" and st.session_state.selected_industry == "General":
                regulation_for_calc = "NPC"
            else:
                regulation_map = {"Qatar": "PDPPL", "India": "DPDP"}
                regulation_for_calc = regulation_map.get(st.session_state.selected_country, st.session_state.selected_regulation)
    ...
    if questionnaire_mismatch:
        ...
        if not st.session_state.get('auto_fixing_mismatch', False):
            ...
            # DEMO MODE: Force NPC for all selections
            regulation_for_calc = "NPC"
            logger.info(f"[DEMO] AUTO-FIXING: FORCING NPC for recalculation")
...
def render_welcome_page():
    ...
    with center_col:
        ...
        with st.container():
            ...
            # Default to 'Qatar' if not set
            selected_country = st.session_state.get("selected_country", "Qatar" if "Qatar" in country_names else country_names[0])
            ...
            # Always reset industry if country changed or current is invalid
            if (
                "last_selected_country" not in st.session_state or
                st.session_state.last_selected_country != selected_country or
                st.session_state.get("selected_industry") not in industry_options
            ):
                st.session_state.selected_industry = industry_options[0] if industry_options else ""
            st.session_state.last_selected_country = selected_country

            # Store the previous industry to detect changes
            previous_industry = st.session_state.get("selected_industry", "")
            ...
            if selected_industry != st.session_state.get("selected_industry"):
                ...
                st.rerun()  # Force immediate rerun to update regulation field

            st.write("")

            # 4. Regulation (auto-populated, disabled)
            # Determine regulation based on current selectbox value (selected_industry), not session state
            if selected_country == "Qatar":
            ...
            st.selectbox(
                ...
                index=0,
                disabled=True,
                key=f"regulation_select_{selected_country}_{selected_industry}",  # Add unique key to force re-render
                help="Auto populated based on the selected country and industry"
            )
            st.session_state.selected_regulation = regulation_key  # Use the determined regulation key
...
def render_privacy_policy_analyzer() -> None:
    ...
    with center_col:
        ...
        with st.container():
            ...
            from privacy_policy_analyzer import PRIVACY_LAWS
            country_options = {config["name"]: key for key, config in PRIVACY_LAWS.items()}
            allowed_countries = ["Qatar", "India", "Qatar (NPC)"]
            country_options = {k: v for k, v in country_options.items() if k in allowed_countries}
            country_names = list(country_options.keys())
            ...
            # st.markdown('<p class="input-label">Industry *</p>', unsafe_allow_html=True)
            # if selected_country == "Qatar":
            #     industry_options = ["Oil and Gas"]
            # elif selected_country == "India":
            #     industry_options = ["Banking and finance", "E-commerce"]
            # else:
            #     industry_options = []
            # # Reset industry if country changed or invalid
            # if (
            #     "ppa_last_selected_country" not in st.session_state or
            #     st.session_state.ppa_last_selected_country != selected_country or
            #     st.session_state.get("ppa_selected_industry") not in industry_options
            # ):
            #     st.session_state.ppa_selected_industry = industry_options[0] if industry_options else ""
            # st.session_state.ppa_last_selected_country = selected_country
            # selected_industry = st.selectbox(
            #     "Industry",
            #     options=industry_options,
            #     key="ppa_selected_industry",
            #     index=industry_options.index(st.session_state.ppa_selected_industry) if st.session_state.ppa_selected_industry in industry_options else 0,
            #     label_visibility="collapsed"
            # )
            # st.write("")
            ...
            st.session_state.ppa_selected_regulation = selected_country_key
            st.write("")
